...
func GetAllFolders(req *FetchFolderRequest) (*FetchFolderResponse, error) { // called by main and gets passed the OrgID via FFR type
	...
	r, _ := FetchAllFoldersByOrgID(req.OrgID) // calling the func, passing the OrgID and storing the output // TODO * use var: err *
	for k, v := range r {                     // for all the key val pairs in r //TODO use _ for throwaway val k
		f = append(f, *v) // put a pointer to each val in f
	}
	var fp []*Folder        // init pointer folder // TODO * use var: fs *
	for k1, v1 := range f { // for each key val pair in f // TODO use _ for throwaway val k1
...
} // TODO work out why the output here is the last JSON obj over and over
